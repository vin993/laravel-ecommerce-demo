...
abstract class AbstractType
{
    ...
    public function totalQuantity()
    {
        if (! $inventoryIndex = $this->getInventoryIndex()) {
            // Fallback to actual inventories if index is not available
            return $this->product->inventories()->sum('qty');
        }
    ...
    public function prepareForCart($data)
    {
        ...
        if (isset($data['cart_id'])) {
            ...
            $isAdminOrder = $cart && $cart->is_active == 0;
        }

        if (! $isAdminOrder && ! $this->haveSufficientQuantity($data['quantity'])) {
