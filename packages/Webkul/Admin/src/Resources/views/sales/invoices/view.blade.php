<x-admin::layouts>
    ...
    <div class="mt-7 flex items-center justify-between gap-4 max-md:flex-wrap">
        <div class="flex flex-wrap items-center gap-x-1 gap-y-2">
            ...
            <a
                href="{{ route('admin.sales.invoices.view_pdf', $invoice->id) }}"
            ...
            <div class="box-shadow rounded bg-white dark:bg-gray-900">
                                ...
                                <div class="grid place-content-start gap-1.5">
                                    ...
                                    <p class="break-all text-base font-semibold text-gray-800 dark:text-white">
                                        ...
                                        @if(isset($item->additional['ari_partstream']) && $item->additional['ari_partstream'])
                                            <span class="ml-2 text-xs font-semibold" style="color: #E13124;">ARI PartStream</span>
                                        @endif
                                    </p>
                                    ...
                                    <div class="flex flex-col place-items-start gap-1.5">
                                        ...
                                        <p class="text-gray-600 dark:text-gray-300">
                                            @if(isset($item->additional['ari_partstream']) && $item->additional['ari_partstream'])
                                                @lang('admin::app.sales.invoices.view.sku', ['sku' => $item->additional['sku']])
                                            @elseif($item->product)
                                                @lang('admin::app.sales.invoices.view.sku', ['sku' => $item->getTypeInstance()->getOrderedItem($item)->sku])
                                            @else
                                                @lang('admin::app.sales.invoices.view.sku', ['sku' => $item->sku])
                                            @endif
                                        </p>
                ...
                <div class="mt-4 flex w-full justify-end gap-2.5 p-4">
                    <div class="flex flex-col gap-y-1.5">
                        ...
                        @php
                            $totalDueAmount = $invoice->base_grand_total - $invoice->order->base_grand_total_invoiced;
                        @endphp
                    ...
                    <div class="flex flex-col gap-y-1.5">
                        ...
                        @if(isset($invoice->additional_shipping_amount) && $invoice->additional_shipping_amount > 0)
                            <p class="!leading-5 text-gray-600 dark:text-gray-300">
                                {{ core()->formatBasePrice($invoice->additional_shipping_amount) }}
                            </p>
                        @endif
                        ...
                        @if($totalDue > 0)
                            <!-- Total Paid -->
                            <p class="!leading-5 text-gray-600 dark:text-gray-300">
                                {{ core()->formatBasePrice($invoice->order->base_grand_total_invoiced) }}
                            </p>
