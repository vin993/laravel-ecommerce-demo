                ...
                <x-admin::drawer ref="invoice">
                    ...
                    <x-slot:content class="!p-0">
                        <div class="grid p-4 !pt-0">
                            ...
                            @php
                                $ariItems = $order->ariPartstreamItems()->get();
                            @endphp

                            @if($ariItems->count() > 0)
                                <div class="px-4 py-2 mt-4" style="background: linear-gradient(135deg, #E13124 0%, #c12a1e 100%);">
                                    <p class="text-sm font-bold text-white flex items-center gap-2">
                                        <span class="icon-settings text-lg"></span>
                                        ARI PartStream Items
                                    </p>
                                </div>

                                @foreach ($ariItems as $ariItem)
                                    <div class="flex justify-between gap-2.5 border-b border-slate-300 py-4 dark:border-gray-800" style="background: #fef2f2;">
                                        <div class="flex gap-2.5">
                                            <div class="relative h-[60px] max-h-[60px] w-full max-w-[60px] rounded border border-dashed border-gray-300 dark:border-gray-800 dark:mix-blend-exclusion dark:invert">
                                                <img src="{{ bagisto_asset('images/product-placeholders/front.svg') }}">
                                                <p class="absolute bottom-1.5 w-full text-center text-[6px] font-semibold text-gray-400">
                                            ...
                                            <div class="grid place-content-start gap-1.5">
                                                ...
                                                <div class="flex flex-col place-items-start gap-1.5">
                                                    ...
                                                    <p class="text-gray-600 dark:text-gray-300">
                                                        @lang('admin::app.sales.invoices.create.sku', ['sku' => $ariItem->sku])
                                                    </p>
                                        ...
                                        <div class="grid ltr:text-right rtl:text-left">
                                            <x-admin::form.control-group>
                                                ...
                                                <x-admin::form.control-group.control
                                                    type="text"
                                                    class="!w-[100px]"
                                                    :id="'invoice[ari_items][' . $ariItem->id . ']'"
                                                    :name="'invoice[ari_items][' . $ariItem->id . ']'"
                                                    rules="required|numeric|min:0"
                                                    :value="$ariItem->quantity"
                                                    :label="trans('admin::app.sales.invoices.create.qty-to-invoiced')"
                                                    :placeholder="trans('admin::app.sales.invoices.create.qty-to-invoiced')"
                                                />

                                                <x-admin::form.control-group.error :control-name="'invoice[ari_items][' . $ariItem->id . ']'" />
                                            </x-admin::form.control-group>
                                        </div>
                                    </div>
                                @endforeach
