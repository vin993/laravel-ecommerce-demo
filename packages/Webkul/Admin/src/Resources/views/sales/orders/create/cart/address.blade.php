        ...
        app.component('v-cart-addresses', {
            ...
            methods: {
                    ...
                    return this.$axios.post('{{ route('admin.customers.customers.addresses.store', $cart->customer_id) }}', params)
                        ...
                        .catch(error => {
                            ...
                            if (error.response?.status == 422) {
                                ...
                                // Show flash message with validation error
                                let errorMessage = error.response.data.message || 'Please fix the validation errors';
                                this.$emitter.emit('add-flash', { type: 'error', message: errorMessage });
                            } else {
                                // Show error message for non-422 errors
                                let message = error.response?.data?.message || error.message || 'Error creating address';
                ...
                updateCustomerAddress(id, params, { setErrors }) {
                    ...
                    // Ensure default_address is 0 or 1 (validation requires in:0,1)
                    params['default_address'] = this.selectedAddressForEdit.default_address ? 1 : 0;
                    ...
                    return this.$axios.put('{{ route('admin.customers.customers.addresses.update', ':id') }}'.replace(':id', id), params)
                        ...
                        .catch(error => {
                            ...
                            if (error.response?.status == 422) {
                                ...
                                // Show error message for non-422 errors
                                let message = error.response?.data?.message || error.message || 'Error updating address';
                                this.$emitter.emit('add-flash', { type: 'error', message: message });
                            }
                        ...
                        .catch(error => {
                            ...
                            if (error.response?.status == 422) {
                                ...
                                if (error.response.data.errors) {
                                    // Set all validation errors from the response
                                    setErrors(error.response.data.errors);

                                    // Also show a flash message with the main error
                                    let errorMessage = error.response.data.message || 'Please fix the validation errors';
                                    this.$emitter.emit('add-flash', { type: 'error', message: errorMessage });
                                } else {
                                    // Fallback to old behavior if no detailed errors
                                ...
                                // Show error message for non-422 errors
                                let message = error.response?.data?.message || error.message || 'Error saving address';
                                this.$emitter.emit('add-flash', { type: 'error', message: message });
                            }
                        });
                },
