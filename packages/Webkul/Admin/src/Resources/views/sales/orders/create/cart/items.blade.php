            ...
            methods: {
                ...
                addToCart(params) {
                    ...
                    if (!product) {
                        this.$emitter.emit('add-flash', {
                            type: 'error',
                            message: 'Product not found. Please search again.'
                        });
                        return;
                    }
                ...
                updateItem(item, qty) {
                    ...
                    this.$axios.put("{{ route('admin.sales.cart.items.update', $cart->id) }}", params)
                        .then(response => {
                            if (response.data.data && response.data.data.items !== undefined) {
                                this.$emit('cart-item-updated', response.data.data);

                                this.$emitter.emit('add-flash', { type: 'success', message: response.data.message });
                            } else {
                                // Handle error in response (when exception is caught but returned as 200)
                                let errorMessage = response.data.message || 'Error updating cart item';

                                this.$emitter.emit('add-flash', { type: 'error', message: errorMessage });

                                // Reload cart to reset to previous state
                                this.$parent.getCart();
                            }
                        ...
                        .catch(error => {
                            ...
                            let message = 'Error updating cart item';

                            if (error.response?.data) {
                                // Try to get message from different locations
                            ...
                            // Reload cart after a short delay to reset to previous state
                            // This allows the user to see the error message
                            setTimeout(() => {
                                this.$parent.getCart();
                            }, 100);
                        });
