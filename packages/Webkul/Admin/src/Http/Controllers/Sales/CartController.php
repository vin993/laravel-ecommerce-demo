...
class CartController extends Controller
{
    ...
    public function store(): JsonResource|JsonResponse
    {
        ...
        try {
            // Check if customer already has an inactive cart
            $existingCart = $this->cartRepository->findOneWhere([
                'customer_id' => $customer->id,
                'is_active'   => false,
            ]);

            if ($existingCart) {
                // Reuse existing inactive cart
                $cart = $existingCart;
            } else {
                // Create new cart
     ...
     */
    public function updateItem(int $cartId): JsonResource|JsonResponse
    {
        $cart = $this->cartRepository->findOrFail($cartId);
