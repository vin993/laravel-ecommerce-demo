...
class OrderDataGrid extends DataGrid
{
    ...
    public function prepareQueryBuilder()
    {
        $queryBuilder = DB::table('orders')
            ->leftJoin(DB::raw('(SELECT order_id, city, state, country FROM addresses WHERE address_type = "'.OrderAddress::ADDRESS_TYPE_SHIPPING.'" GROUP BY order_id) as order_address_shipping'), 'order_address_shipping.order_id', '=', 'orders.id')
            ->leftJoin(DB::raw('(SELECT order_id, city, state, country FROM addresses WHERE address_type = "'.OrderAddress::ADDRESS_TYPE_BILLING.'" GROUP BY order_id) as order_address_billing'), 'order_address_billing.order_id', '=', 'orders.id')
            ...
            ->groupBy('orders.id', 'order_payment.method', 'orders.increment_id', 'orders.base_grand_total', 'orders.created_at', 'orders.channel_name', 'orders.channel_id', 'orders.status', 'orders.customer_email', 'orders.cart_id', 'orders.customer_first_name', 'orders.customer_last_name', 'order_address_billing.city', 'order_address_billing.state', 'order_address_billing.country');
