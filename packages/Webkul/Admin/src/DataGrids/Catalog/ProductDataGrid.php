...
class ProductDataGrid extends DataGrid
{
    ...
    public function prepareQueryBuilder()
    {
        ...
        $queryBuilder = DB::table('product_flat')
            ...
            // Join child products for configurable products
            ->leftJoin('products as child_products', function ($join) {
                $join->on('product_flat.product_id', '=', 'child_products.parent_id');
            })
            // Join child product inventories
            ->leftJoin('product_inventories as child_inventories', 'child_products.id', '=', 'child_inventories.product_id')
            ...
            // Sum both direct inventory and child inventories
            ->addSelect(DB::raw('COALESCE(SUM(DISTINCT '.$tablePrefix.'product_inventories.qty), 0) + COALESCE(SUM('.$tablePrefix.'child_inventories.qty), 0) as quantity'))
        ...
        foreach ($params as $attribute => $value) {
            ...
            if ($attribute == 'all') {
                ...
                foreach ($value as $searchTerm) {
                    $searchFilters['bool']['should'][] = [
                        'bool' => [
                            'should' => [
                                ['match_phrase_prefix' => ['name' => $searchTerm]],
                                ['match_phrase_prefix' => ['sku' => $searchTerm]],
                            ],
                        ],
                    ];
                }
