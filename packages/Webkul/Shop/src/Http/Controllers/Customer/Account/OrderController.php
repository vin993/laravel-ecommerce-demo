...
class OrderController extends Controller
{
    ...
    public function view($id)
    {
        ...
        $oemItems = collect();
        if ($order->additional && isset($order->additional['oem_items'])) {
            $oemItems = collect($order->additional['oem_items'])->map(function($item) {
                $product = null;
                if (isset($item['product_id'])) {
                    $product = \Webkul\Product\Models\Product::with('images')->find($item['product_id']);
                }

                return array_merge($item, [
                    'product' => $product,
                    'image_url' => $product && $product->images->isNotEmpty()
                        ? asset('storage/' . $product->images->first()->path)
                        : asset('themes/maddparts/images/placeholder.jpg'),
                    'product_url' => $product && $product->url_key
                        ? url($product->url_key)
                        : null
                ]);
            });
        ...
        return view('shop::customers.account.orders.view', compact('order', 'oemItems', 'ariItems'));
    }
