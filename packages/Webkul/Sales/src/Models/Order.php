...
class Order extends Model implements OrderContract
{
    ...
    /**
     * Get the ARI PartStream items associated with the order.
     */
    public function ariPartstreamItems(): HasMany
    {
        return $this->hasMany(\App\Models\AriPartstreamOrderItem::class, 'order_id');
    }
    ...
    public function canShip(): bool
    {
        ...
        if ($this->ariPartstreamItems()->count() > 0 && ! in_array($this->status, [self::STATUS_CLOSED, self::STATUS_FRAUD])) {
            return true;
        }
    ...
    {
        // Allow cancellation for pending_payment orders (customer hasn't paid yet)
        ...
        if ($this->ariPartstreamItems()->count() > 0 && ! in_array($this->status, [self::STATUS_CLOSED, self::STATUS_FRAUD, self::STATUS_CANCELED])) {
            return true;
        }
