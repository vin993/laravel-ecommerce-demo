...
class OrderRepository extends Repository
{
    ...
    public function cancel($orderOrId)
    {
        ...
        // Special handling for pending_payment orders - force cancel all items
        if ($order->status === \Webkul\Sales\Models\Order::STATUS_PENDING_PAYMENT) {
            foreach ($order->items as $item) {
                ...
                foreach ($orderItems as $orderItem) {
                    // Return inventory regardless of invoice status
                    $this->orderItemRepository->returnQtyToProductInventory($orderItem);

                    // Cancel the full ordered quantity (not just qty_to_cancel)
            ...
            // Normal cancellation logic for other order statuses
            foreach ($order->items as $item) {
