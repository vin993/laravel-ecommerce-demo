...
class InvoiceRepository extends Repository
{
    ...
    public function create(array $data, $invoiceState = null, $orderState = null)
    {
        ...
        try {
            ...
            $regularItemsQty = isset($data['invoice']['items']) ? array_sum($data['invoice']['items']) : 0;
            $ariItemsQty = isset($data['invoice']['ari_items']) ? array_sum($data['invoice']['ari_items']) : 0;
            $totalQty = $regularItemsQty + $ariItemsQty;
            ...
            if (isset($data['email_sent'])) {
                $invoiceCreateData['email_sent'] = $data['email_sent'];
            }

            $invoice = $this->model->create($invoiceCreateData);
            ...
            if (isset($data['invoice']['items'])) {
                ...
                $invoiceItem = $this->invoiceItemRepository->create([
                    ...
                    'price_incl_tax'       => $qty > 0 ? $orderItem->price + ($taxAmount / $qty) : $orderItem->price,
                    'base_price'           => $orderItem->base_price,
                    'base_price_incl_tax'  => $qty > 0 ? $orderItem->base_price + ($baseTaxAmount / $qty) : $orderItem->base_price,
            ...
            if (isset($data['invoice']['ari_items'])) {
                foreach ($data['invoice']['ari_items'] as $ariItemId => $qty) {
                    if (! $qty) {
                        continue;
                    }

                    $ariItem = \DB::table('ari_partstream_order_items')->where('id', $ariItemId)->first();
                    ...
                    if (isset($ariItem->tax_amount) && $ariItem->tax_amount > 0) {
                        $taxAmount = round((($ariItem->tax_amount / $ariItem->quantity) * $qty), 4);
                        $baseTaxAmount = round((($ariItem->base_tax_amount / $ariItem->quantity) * $qty), 4);
                    } else {
                        $itemTotal = $ariItem->price * $qty;
                        $allAriItems = \DB::table('ari_partstream_order_items')->where('order_id', $order->id)->get();
                        $totalAriItemsSubtotal = $allAriItems->sum(function($item) {
                        ...
                        foreach ($order->items as $orderItem) {
                            if ($orderItem->parent_id === null) {
                                $regularItemsTax += $orderItem->base_tax_amount;
                            }
                        }

                        $ariItemsTax = $order->base_tax_amount - $regularItemsTax;

                        if ($totalAriItemsSubtotal > 0 && $ariItemsTax >= 0) {
                            $ariTaxRatio = $ariItemsTax / $totalAriItemsSubtotal;
                            $taxAmount = round($itemTotal * $ariTaxRatio, 4);
                            $baseTaxAmount = round($itemTotal * $ariTaxRatio, 4);
                        } elseif ($order->base_sub_total > 0) {
                            $taxRatio = $order->base_tax_amount / $order->base_sub_total;
                            $taxAmount = round($itemTotal * $taxRatio, 4);
                            $baseTaxAmount = round($itemTotal * $taxRatio, 4);
                        }
                    }

                    $priceInclTax = $qty > 0 ? round($ariItem->price + ($taxAmount / $qty), 4) : $ariItem->price;
                    $basePriceInclTax = $qty > 0 ? round($ariItem->price + ($baseTaxAmount / $qty), 4) : $ariItem->price;
                    $totalInclTax = round(($ariItem->price * $qty) + $taxAmount, 4);
                    $baseTotalInclTax = round(($ariItem->price * $qty) + $baseTaxAmount, 4);
                    ...
                    $this->invoiceItemRepository->create([
                        ...
                        'base_price'           => $ariItem->price,
                        'base_price_incl_tax'  => $basePriceInclTax,
                        'total_incl_tax'       => $totalInclTax,
                        'total'                => $ariItem->price * $qty,
                        'base_total'           => $ariItem->price * $qty,
                        'base_total_incl_tax'  => $baseTotalInclTax,
                        'tax_amount'           => $taxAmount,
                        'base_tax_amount'      => $baseTaxAmount,
                        'discount_amount'      => 0,
                        'base_discount_amount' => 0,
                        ...
                        'additional'           => json_encode([
                            'ari_partstream' => true,
                            'brand' => $ariItem->brand,
                            'supplier' => $ariItem->selected_supplier,
                            'sku' => $ariItem->sku,
                        ]),
                    ]);
                }
    ...
    public function collectTotals($invoice)
    {
        ...
        // Include additional shipping amount if it exists and has been paid
        $invoice->additional_shipping_amount = 0;
        if (isset($invoice->order->additional_shipping_amount) &&
            isset($invoice->order->additional_shipping_invoice_status) &&
            $invoice->order->additional_shipping_invoice_status === 'paid') {
            $invoice->additional_shipping_amount = $invoice->order->additional_shipping_amount;
        }
        ...
        $invoice->grand_total = $invoice->sub_total + $invoice->tax_amount + $invoice->shipping_amount + $invoice->additional_shipping_amount - $invoice->discount_amount;
        $invoice->base_grand_total = $invoice->base_sub_total + $invoice->base_tax_amount + $invoice->base_shipping_amount + $invoice->additional_shipping_amount - $invoice->base_discount_amount;

        $order = $invoice->order;
        $totalInvoicedQty = $invoice->total_qty;
        $totalOrderQty = $order->items->where('parent_id', null)->sum('qty_ordered');
        $ariItemsCount = \DB::table('ari_partstream_order_items')->where('order_id', $order->id)->sum('quantity');
        $totalQty = $totalOrderQty + $ariItemsCount;

        $isFirstInvoice = $order->invoices()->where('id', '!=', $invoice->id)->count() == 0;

        if ($isFirstInvoice && abs($totalInvoicedQty - $totalQty) < 0.01) {
            $taxDifference = $order->base_tax_amount - $invoice->base_tax_amount;

            if (abs($taxDifference) > 0 && abs($taxDifference) < 5) {
                $invoice->tax_amount += $taxDifference;
                $invoice->base_tax_amount += $taxDifference;
                $invoice->grand_total += $taxDifference;
                $invoice->base_grand_total += $taxDifference;
            }
        }
