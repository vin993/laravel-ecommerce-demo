...
Route::group(['prefix' => 'customer', 'middleware' => ['web']], function () {
    ...
    Route::post('/register', function (Request $request) {
        try {
            ...
            return redirect('/')->with('success', 'Account created successfully! Welcome to ' . config('app.name', 'Madd Parts') . '!');
...
// Dynamic slug route - Updated to skip checkout
Route::get('/{slug}', function ($slug) {
    $skipPaths = ['api', 'admin', 'storage', 'cart', 'customer', 'checkout', 'wishlist', 'brands', 'search', 'products'];
    if (in_array($slug, $skipPaths)) {
        abort(404);
    }

    $product = \Webkul\Product\Models\ProductFlat::where('url_key', $slug)
        ->where('channel', 'maddparts')
        ->where('locale', 'en')
        ->first();

    if ($product) {
        return app(ProductController::class)->view($slug);
    }

    return app(CategoryController::class)->view($slug, request());
