...
class SyncSuccessNotification extends Mailable
{
    ...
    {
        $subject = 'Automated FTP Sync Completed - ' . $this->syncLog->sync_date->format('M d, Y');

        return $this->subject($subject)
            ->view('emails.automated-sync.success')
            ->with([
                'syncLog' => $this->syncLog,
                'duration' => gmdate('H:i:s', $this->syncLog->total_duration_seconds),
                'filesDetected' => count($this->syncLog->update_files_detected ?? []),
                'filesProcessed' => count($this->syncLog->update_files_processed ?? []),
            ]);
    }
}
