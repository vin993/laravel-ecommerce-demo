...
class HomeController extends Controller
{
    ...
    public function index()
    {
        ...
        try {
            ...
            // Ensure it's a collection even if getAllActive returns null
            if (!$cmsContent || !($cmsContent instanceof \Illuminate\Support\Collection)) {
                $cmsContent = collect();
            }
            
            // Ensure it's keyed by section_key
            if ($cmsContent->isEmpty() || !$cmsContent->has('hero_section')) {
                $cmsContent = $cmsContent->keyBy('section_key');
            }
        } catch (\Exception $e) {
    ...
    {
        try {
            // Get best selling ARI/OEM items from ari_partstream_order_items
            $bestSellers = \DB::table('ari_partstream_order_items as ari')
                ->select(
                    'ari.sku',
                    'ari.name',
                    'ari.brand',
                    'ari.price',
                    'ari.image_url',
                    \DB::raw('SUM(ari.quantity) as total_sold'),
                    \DB::raw('MAX(ari.return_url) as url_key')
                )
                ->join('orders', 'orders.id', '=', 'ari.order_id')
                ->where('orders.status', '!=', 'canceled')
                ->where('orders.status', '!=', 'closed')
                ->groupBy('ari.sku', 'ari.name', 'ari.brand', 'ari.price', 'ari.image_url')
                ->orderBy('total_sold', 'desc')
                ->limit(8)
                ->get()
                ->map(function ($item) {
                    // Format the data to match the view expectations
                    return (object) [
                        'sku' => $item->sku,
                        'name' => $item->name,
                        'brand' => $item->brand,
    ...
    public function contact()
    {
        ...
        try {
            ...
            \Log::error('Contact page CMS content error: ' . $e->getMessage());
            $cmsContent = collect()->keyBy('section_key');
        }
        
        return view('home.contact', compact('cmsContent'));
    }
    ...
    return Cache::remember('header_categories', 86400, function () {
        ...
        foreach ($oemBrands as $brand) {
            $categories[] = [
                'id' => null,
                'name' => $brand['name'],
                'slug' => strtolower($brand['code']),
                'url' => route('ari.partstream') . '?aribrand=' . $brand['code'],
                'children' => []
            ];
        }
